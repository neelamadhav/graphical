...
# for ner
classes_list = ['person', 'geo-loc', 'facility', 'company','product', 'other','O']
classes = {}
index = 0
...
for i in range(num_lines_user):
	...
	if(len(s_user)<=1):
		...
		user_label = s_user[1]
		gold_label = s_gold[1]
		user_label = re.sub("^B-","",user_label);
		user_label = re.sub("^I-","",user_label)
		gold_label = re.sub("^B-","",gold_label);
		gold_label = re.sub("^I-","",gold_label)
